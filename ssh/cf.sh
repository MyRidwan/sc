...
BURIQ () {
    curl -sS https://myridwan.github.io/izin > /root/tmp
...
Name=$(curl -sS https://myridwan.github.io/izin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://myridwan.github.io/izin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
